...
class PromptScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => PromptCubit(ApiService(AppConfig.baseUrl)),
...
class _PromptScreenBodyState extends State<_PromptScreenBody> {
  ...
  Widget _buildCurrentScreen(BuildContext context, PromptState state) {
    return switch (state.screenState) {
      ...
      PromptScreenState.showImage =>
          _buildImageDisplay(context, state.imageFile),
  ...
  Widget _buildImageDisplay(BuildContext context, File? imageFile) {
    return Padding(
      padding: const EdgeInsets.all(20),
      child: Column(
        children: [
